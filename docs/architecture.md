...
- Edge Function `grade-exam-answer`: deployed auf dem armserver unter
  **`/opt/supabase/volumes/functions/grade-exam-answer/index.ts`** (Achtung: die Kopie unter
  ...
  /home/deno/functions/<name>" zeigt die echte Quelle). Der Pfad ist root-owned: neue Version
  per `sudo cp` einspielen, vorher Backup der alten Datei, danach Container
  `supabase-edge-functions` neu starten. Env am Container `supabase-edge-functions`:
  `AWS_BEDROCK_API_KEY` (aus Vaultwarden), `VERIFY_JWT=true` (Main-Service `main/index.ts`
