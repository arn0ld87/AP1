...
- Edge Function `grade-exam-answer`: deployed unter
  ...
  `supabase-edge-functions` neu starten); Env am Container `functions`:
  `AWS_BEDROCK_API_KEY` (aus Vaultwarden), `VERIFY_JWT=true` (Bounce-Main verifiziert
  User-JWTs gegen JWKS).
